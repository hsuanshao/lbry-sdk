...
def staked_support_subquery(claim_hash, aggregate):
    ...
    return (
        select(
            ...
            ((content.c.claim_hash == claim_hash) | (content.c.channel_hash == claim_hash)) &
...
def update_stakes(blocks: Tuple[int, int], claims: int, p: ProgressContext):
    ...
    sql = (
        ...
        .values(
            staked_amount=(
                    Claim.c.amount + claims_in_channel_amount_calc(Claim.c.claim_hash) +
